 ...
 */
class RestSocialAuthController extends Silhouette[User2, JWTAuthenticator] with HeaderEnvironmentModule {
  /**
  ...
  def link(provider: String) = SecuredAction.async(parse.json) { implicit request =>
              Future.successful(Ok(Json.toJson(Good(message = "link with social completed!"))))
/*
