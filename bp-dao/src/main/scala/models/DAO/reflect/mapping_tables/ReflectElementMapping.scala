...
class ReflectElementMappings(tag: Tag) extends Table[ReflectElementMap](tag, "reflect_element_mappings") {
  ...
  def ref_topo_element = column[Option[Int]]("ref_topo_element")
  ...
  def * = (id.?, reflection,
          topology_element, ref_topo_element,
  ...
  def ref_topo_elementFK = foreignKey("reflect_element_mappings_ref_topo_element_fk", 
      ref_topo_element, models.DAO.reflect.ReflectElemTopologDAOF.reflected_elem_topologs)(_.id, onDelete = ForeignKeyAction.SetNull)
