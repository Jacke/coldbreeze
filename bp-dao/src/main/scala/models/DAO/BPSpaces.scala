...
object BPSpaceDAOF {
  ...
  val bpspaces = BPSpaceDAO.bpspaces

  private def filterQuery(id: Int): Query[BPSpaces, BPSpaceDTO, Seq] =
    bpspaces.filter(_.id === id)
  private def filterByBPQuery(id: Int): Query[BPSpaces, BPSpaceDTO, Seq] =
